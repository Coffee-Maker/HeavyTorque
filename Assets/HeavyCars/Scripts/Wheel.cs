...
public class Wheel : VehicleNodeWithTorque {
#if UNITY_EDITOR && !COMPILER_UDONSHARP
    public static Texture2D WheelIcon => Resources.Load<Texture2D>("HeavyCars/UI/Wheel");
#endif
    ...
    public override void Tick(float deltaTime) {
        ...
        if (suspension.contacting) {
            ...
            _longitudinalSlipRatio = tireRoadDifference / Max(1e-5f, Abs(longitudinalAxleSpeed));
        ...
        if (Abs(AngularVelocity) > 0) {
            var breakTorque        = brakeEngagement * brakeForce;
