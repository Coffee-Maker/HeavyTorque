...
public class Engine : VehicleNodeWithTorque {
    ...
    public static Texture2D EngineIcon => Resources.Load<Texture2D>("HeavyCars/UI/Engine");
#endif
    ...
    public float Torque => SimpleThrottleModel(throttleInput);

    public override void Tick(float deltaTime) {
        var idleThrottle                = 0f;
        if (Rpm < idleRpm) idleThrottle = 1 - Rpm / idleRpm + 0.5f;
        var throttle                    = Rpm > limitRpm ? 0 : Lerp(idleThrottle, 1, throttleInput);
